...
const NavBar = () => {
    ...
    return (
        <Navbar className='bg-transparent fixed z-10 w-full p-6'>
            ...
            <NavbarCollapse className='w-full transition-all duration-200'>
                {
                    content.map((item) => {
                        return (
                            <NavbarLink className='transition-all duration-300 inline-block font-bebas py-0 uppercase text-white text-2xl mx-6 lg:mx-8 hover:bg-slate-600 md:hover:text-white dark:text-white dark:hover:text-white hover:ring-1 rounded-xl hover:ring-yellow-400 md:p-2.5' as={Link} href={item.link} key={item.title}>
                                {item.title}
                            </NavbarLink>
                        )
                    })
