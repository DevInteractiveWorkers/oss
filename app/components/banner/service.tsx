...
const ServiceBanner = ({ image, direction, content }: { image: string, direction: boolean, content:{title:string, subtitle:string} }) => {
    return (
        <section className='text-center sm:text-justify relative w-full overflow-hidden my-8 sm:my-0'>
            <article className={`flex items-center justify-evenly relative z-10 ${direction ? 'flex-col sm:flex-row-reverse' : 'flex-col sm:flex-row'}`}>
                <div className='basis-[50%] xl:basis-[40%] md:mt-24'>
                ...
                <div className='basis-[38%]'>
                    <Image className=' aspect-auto' src={image} alt='On service guard' width={417} height={558} ></Image>
                </div>
            </article>
